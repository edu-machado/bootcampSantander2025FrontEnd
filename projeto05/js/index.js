document.addEventListener('DOMContentLoaded', function() {
    const iconesServidor = document.querySelectorAll('.icone-servidor');
    iconesServidor.forEach(icone => {
    ...
    const avatarsUsuario = document.querySelectorAll('.avatar-usuario');
    avatarsUsuario.forEach((avatar, index) => {
        setTimeout(() => {
            avatar.style.border = '4px solid #43b581';
            avatar.style.boxShadow = '0 0 0 4px rgba(67, 181, 129, 0.3)';
        }, index * 500);
    });

    const mensagensChat = document.querySelectorAll('.mensagem-chat');
    mensagensChat.forEach((mensagem, index) => {
        setTimeout(() => {
